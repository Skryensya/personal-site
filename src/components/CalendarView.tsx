...
export function CalendarView({ username = "skryensya", className = "" }: CalendarViewProps) {
  ...
  const canNavigateNext = () => {
    const now = new Date();
    return calendar.currentYear < now.getFullYear() || 
           (calendar.currentYear === now.getFullYear() && calendar.currentMonth < now.getMonth());
  };
  ...
  const handleDayClick = (event: React.MouseEvent, day: any) => {
  ...
  const getMonthStats = () => {
    ...
    const maxDay = monthDays.reduce((max, day) => 
      day.contribution.count > max.contribution.count ? day : max, monthDays[0]
    );
              ...
              <div className="flex gap-1">
                {[0, 1, 3, 6, 12, 25].map((count, index) => (
                  <div
                    ...
                    style={getContributionStyle(count)}
                  />
                ))}
              </div>
              <span>Más</span>
