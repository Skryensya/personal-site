    ...
    <div ref={containerRef} className={`relative ${className}`} style={{ height: `${steps.length * 100}vh` }}>
        ...
        <div className="relative w-full max-w-2xl mx-auto px-6">
          ...
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                <div 
                  className={`w-full max-w-lg border-4 border-main p-8 ${
                    isActive ? 'bg-main text-secondary' : 'bg-secondary text-main'
                  }`}
                >
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="font-mono text-lg font-bold uppercase tracking-wider">
                        {step.year}
                      </div>
                    </div>
                  ...
                  {/* Título */}
                  <h3 className="font-mono text-2xl font-bold mb-4 uppercase tracking-wide">
                    {step.title}
                  </h3>
                    ...
                    {step.tech.map((tech, techIndex) => (
                      <span
                        ...
                        className={`px-3 py-1 font-mono text-xs font-semibold border-2 ${
                          isActive
                            ? 'bg-secondary text-main border-secondary'
                            : 'bg-main text-secondary border-main'
                        }`}
                      >
                        {tech}
                      </span>
      ...
      <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50">
        <div className="bg-secondary border-2 border-main px-6 py-3">
          <div className="font-mono text-xs text-main uppercase tracking-wider mb-2 text-center">
          ...
          <div className="flex gap-2">
            {steps.map((_, index) => (
              <div
                key={index}
                className={`w-3 h-3 border border-main transition-all duration-300 ${
                  index <= currentStep ? 'bg-main' : 'bg-transparent'
                }`}
              />
            ))}
          </div>
