...
export function applyTheme(themeId, isDark = false) {
    ...
    const resolvedThemeId = rawTheme?.id || themes[0]?.id || 'arctic';
...
export function loadThemeFromStorage() {
    ...
    try {
        ...
        if (savedThemeId) {
            ...
            const fallbackThemeId = availableThemes[0]?.id || 'arctic';
            ...
            const availableThemes = getAvailableThemes();
            return { themeId: availableThemes[0]?.id || 'arctic', isDark: false, mode: 'light' };
        }
    } catch (e) {
        return { themeId: 'arctic', isDark: false, mode: 'light' };
    }
}
