...
function runtimeSvg(raw, whiteColor = 'var(--avatar-white)') {
  return raw
    .replaceAll('fill="#ffffff"', `fill="${whiteColor}"`)
...
function previewSvg(raw) {
  return raw
    .replaceAll('fill="#ffffff"', 'fill="#f2f2f2"')
...
function svgInner(raw, transform) {
...
function buildManifest(bundle, tiles, width, height) {
  return {
    name: bundle.name,
...
function getPreviewTileGroup(name) {
  ...
  if (name.startsWith('base-tile-00') || name.startsWith('base-tile-01') || name.startsWith('base-tile-02') || name.startsWith('base-tile-03') || name.startsWith('base-tile-04') || name.startsWith('base-tile-05') || name.startsWith('base-tile-06') || name.startsWith('base-tile-07') || name.startsWith('base-tile-08') || name.startsWith('base-tile-09') || name.startsWith('base-tile-10')) return 2;
  if (name.includes('sweater-alt') || name.startsWith('base-tile-11') || name.startsWith('base-tile-12') || name.startsWith('base-tile-13') || name.startsWith('base-tile-14') || name.startsWith('base-tile-15') || name.startsWith('base-tile-16') || name.startsWith('base-tile-17') || name.startsWith('base-tile-18') || name.startsWith('base-tile-19') || name.startsWith('base-tile-20') || name.startsWith('base-tile-21') || name.startsWith('base-tile-22')) return 3;
  return 4;
}
...
function buildTilesetSvg(bundle, tiles) {
  ...
  const columns = Math.max(1, bundle.columns ?? 6);
  const tileSize = bundle.tileSize ?? 25;
  const padding = bundle.padding ?? 8;
  const rows = Math.ceil(previewTiles.length / columns);
  ...
  const themedSymbols = tiles
    .map(
      (tile) =>
        `<symbol id="${bundle.symbolPrefix}-${tile.name}" viewBox="${tile.viewBox}">${tile.themedInner}</symbol>`
    )
    .join('');
  ...
  const previewSymbols = tiles
    .map(
      (tile) =>
        `<symbol id="${bundle.symbolPrefix}-preview-${tile.name}" viewBox="${tile.viewBox}">${tile.previewInner}</symbol>`
    )
    .join('');
...
async function buildBundle(bundle) {
  const filePaths = (await walk(bundle.sourceDir))
  ...
  for (const filePath of filePaths) {
    ...
    tiles.push({
      ...
      themedInner: svgInner(raw, runtimeSvg),
      previewInner: svgInner(raw, previewSvg),
      isBlank: !raw.includes('#000000') && !raw.includes('#808080') && !raw.includes('#fefefe'),
    });
  }

  const { svg, width, height } = buildTilesetSvg(bundle, tiles);
  const manifest = buildManifest(bundle, tiles, width, height);
