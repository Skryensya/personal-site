...
const workerPath = '/dither-gpu.js';

// Global cache for dithered images
const DITHER_CACHE = new Map();
const MAX_CACHE_SIZE = 20; // Limit memory usage
const CACHE_CLEANUP_THRESHOLD = 25;

// Generate unique cache key for image + settings
function generateCacheKey(imageSrc, width, height, crunchFactor, cutoff, darkrgba, lightrgba) {
    return `${imageSrc}_${width}x${height}_${crunchFactor}_${cutoff}_${darkrgba.join(',')}_${lightrgba.join(',')}`;
}

// Clean up cache when it gets too large
function cleanupCache() {
    if (DITHER_CACHE.size > CACHE_CLEANUP_THRESHOLD) {
        const entries = Array.from(DITHER_CACHE.entries());
...
class ASDitheredImage extends HTMLElement {
    constructor() {
        ...
        this.worker_.onmessage = ((e) => {
            ...
            if (this.current_cache_key_) {
                DITHER_CACHE.set(this.current_cache_key_, {
                    imageData: imageData,
                    timestamp: Date.now()
                });
        ...
        this.last_draw_state_ = { width: 0, height: 0, crunchFactor: 0, imageSrc: '', cacheKey: '' };
    }
    ...
    repaintImage() {
        ...
        const cacheKey = generateCacheKey(
            this.original_image_.currentSrc,
            calculatedWidth,
            calculatedHeight,
            adjustedPixelSize,
            this.cutoff_,
        ...
        if (cached) {
            ...
            this.last_draw_state_.cacheKey = cacheKey;
